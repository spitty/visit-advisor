...
import org.ipccenter.visitadvisor.model.Group;
...
public class TestEntityCreator {
    ...
    public void createTestEntities() {
        ...
        List<Group> userGroups = em.createNamedQuery("Group.getByName", Group.class).setParameter("name", groupName).getResultList();
        ...
        final Group ug = userGroup;
        users.forEach((User t) -> {t.setGroup(ug);});
    ...
    private void persistEntities(List entities) throws NotSupportedException, SystemException, RollbackException, HeuristicMixedException, HeuristicRollbackException {
        try {
            ...
            for (int i = 0; i < entities.size(); i++) {
                ...
                LOG.debug("Entity #{} persisted: {}", i, entities.get(i));
            }
            em.flush();
